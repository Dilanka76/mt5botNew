        ...
        if self.prev_ema13 is not None and self.prev_ema21 is not None:
            ...
            if self.position is not None:
                    ...
                    if immediate or self.pending_reversal_direction == direction:
                        ...
                        if adx_ok:
                            ...
                            if not self._warmed_up():
                                # See the warm-up note in __init__. The close
                                # above still happened; only the re-entry is
                                # withheld, because that cross is history.
                                log_decision(
                                    self.config.symbol, "entry_skipped_warmup",
                                    f"{direction.value} cross at {last_closed_time} ignored for "
                                    f"re-entry: less than one {self.config.timeframe} candle since "
                                    f"startup, so this cross closed before the bot was watching.",
                                )
                ...
                if is_confirmed_cross:
                    ...
                    if not self._warmed_up():
                        # See the warm-up note in __init__. This cross closed
                        # before, or around, the moment this process started,
                        # so it is history, not a signal.
                        log_decision(
                            self.config.symbol, "entry_skipped_warmup",
                            f"{direction.value} cross at {last_closed_time} ignored: the bot has "
                            f"been running less than one {self.config.timeframe} candle, so this "
                            f"cross closed before it was watching. Waiting for a live one.",
                        )
                    elif not is_within_session(self._active_sessions()):
                        log_decision(
                            self.config.symbol, "cross_ignored_outside_session",
