"""dual_cross_confirmed_swap_adx_entrygate ("ADX gates entries too, not
just the swap") — strategy_variant=dual_cross_confirmed_swap_adx_entrygate.

BACKTEST-ONLY as of 2026-08-22 — NEVER register in main.py's
...
Identical to dual_cross_confirmed_swap_adx (the currently-live variant,
see its own docstring for the full base design) except for ONE addition,
built on the user's explicit instruction 2026-08-22: the SAME ADX(14) >=
threshold gate that already protects the swap now ALSO protects fresh
entries from flat. Before either the immediate-entry or the
gap/EMA5-pullback path runs at all, check ADX at the confirming candle —
below threshold, the whole signal is thrown away (no entry, no pending
setup armed), exactly as if no cross had happened. The bot just waits
for the next confirmed cross and checks that one fresh, independently.

Root cause this targets: a real 2026-08-21 trade (SELL, entry 4558.90,
confirming candle ADX=20.40) opened right in the middle of a real,
chart-verified choppy stretch — 7+ EMA13/21 crosses in about an hour,
ADX mostly under 20 the whole time — and (due to a separate, now-fixed
bug) rode the position, unmanaged, into a genuine breakout that reversed
against it, a -$166.14 loss. Traced candle-by-candle with the user: this
exact entry's own confirming-candle ADX (20.40) was already below 25 —
this filter would have blocked it from ever opening.

**Real trade-off, checked directly against today's actual trades, not
just theory** — this is NOT a free improvement:
  - Would have BLOCKED the 4558.90 SELL (ADX 20.40) — the trade that
    caused the loss above. Good.
  - Would have BLOCKED the 10:03 BUY entry (ADX ~20) — but that trade
    actually WON (+$29.64). Real cost.
  - Would have BLOCKED the 12:05 SELL entry (ADX ~18-19) — also WON
    (+$29.64). Real cost.
  - Would NOT have affected the 09:26 SELL entry (ADX ~37-38, already
    passes) — won either way.
So on the small real sample checked so far, this filter blocks 1 real
loss and 2 real wins — net effect unclear without a proper backtest over
more data. Explicitly NOT assumed to be a clean win; built specifically
to be tested, not deployed on faith.

**Deliberately does NOT add a second, different idea that was discussed
and explicitly rejected**: exiting an ALREADY-OPEN position early just
because ADX drops while holding it. A real counter-example (the 10:03
BUY) dipped into a choppy stretch mid-trade (ADX down to ~15) and fully
recovered to hit take-profit — an exit-on-chop rule would have cut that
winner short for no real reason. This variant only ever gates NEW
entries; once a position is open, it's managed exactly the same as
dual_cross_confirmed_swap_adx (stop-loss, swap+ADX gate, take-profit —
...
logger = logging.getLogger("bot.strategy.state_machine_dual_cross_confirmed_swap_adx_entrygate")
...
class DualCrossConfirmedSwapAdxEntrygateEngine:
    def __init__(self, config: AppConfig, connector: MT5Connector, executor: TradeExecutor):
        if config.stop_loss_usd is None:
            raise ValueError(
                "strategy_variant=dual_cross_confirmed_swap_adx_entrygate requires stop_loss_usd to be set."
            )
        if config.swap_adx_filter is None:
            raise ValueError(
                "strategy_variant=dual_cross_confirmed_swap_adx_entrygate requires a "
                "'swap_adx_filter:' config section (now gates entries too, not just the swap)."
            )
        if config.gap_threshold_usd is None:
            raise ValueError(
                "strategy_variant=dual_cross_confirmed_swap_adx_entrygate requires gap_threshold_usd to be set "
    ...
    def _active_sessions(self) -> list:
        return self.config.sessions["dual_cross_confirmed_swap_adx_entrygate"]
            ...
            if self.position is not None:
                ...
                if is_confirmed_cross:
                    ...
                    if not is_within_session(self._active_sessions()):
                        ...
                        # Entry ADX gate (the whole point of this variant
                        # vs plain dual_cross_confirmed_swap_adx) — checked
                        # once, at this confirming candle, before either
                        # entry path (immediate or gap/EMA5-pullback) runs
                        # at all. Below threshold: throw the entire signal
                        # away, as if no cross happened this candle. Does
                        # NOT keep waiting for ADX to rise on this same
                        # signal — the next independent confirmed cross
                        # gets its own fresh check. See module docstring
                        # for the real trade-off this was built to test.
                        adx_value = last_closed["adx"]
                        adx_ok = not math.isnan(adx_value) and adx_value >= self.config.swap_adx_filter.adx_threshold
                        if not adx_ok:
                            log_decision(
                                self.config.symbol, "entry_blocked_low_adx",
                                f"{direction.value} cross confirmed (ema13={ema13:.2f}, ema21={ema21:.2f}) but adx="
                                f"{'nan' if math.isnan(adx_value) else f'{adx_value:.1f}'} < "
                                f"{self.config.swap_adx_filter.adx_threshold:.1f} -> ENTRY BLOCKED, staying flat, "
                                f"waiting for the next independent signal",
                            )
                        else:
